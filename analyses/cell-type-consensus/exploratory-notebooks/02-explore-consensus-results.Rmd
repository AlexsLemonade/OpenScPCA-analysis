...
celltype_distribution_plot <- function(label,
  ...
  plot_df <- results_df |> 
    ...
    dplyr::mutate(
      ...
      top_celltypes = forcats::fct_lump_n(consensus_annotation, 9, other_level = "All remaining cell types", ties.method = "first") |> 
        # sort by frequency 
        forcats::fct_infreq() |> 
        # make sure all remaining and unknown are last 
  ...
  # first figure out how many cell types are assigned to make sure colors are assigned properly 
  num_assigned_celltypes <- plot_df |> 
    dplyr::filter(!top_celltypes %in% c("Unknown")) |> 
  ...
  celltypes <- unique((plot_df$top_celltypes)) |> 
    sort() #make sure they are in the right order 
...
results_files <- list.files(opt$results_dir, pattern = "*_consensus-cell-types.tsv.gz", full.names = TRUE)
