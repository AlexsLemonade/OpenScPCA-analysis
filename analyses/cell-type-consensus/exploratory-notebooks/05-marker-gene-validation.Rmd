...
# and mean gene expression per group of each marker gene and mean expression of all marker genes 
group_stats_df <- combined_df |> 
  ...
  dplyr::mutate(
    ...
    broad_celltype_group = dplyr::if_else(is.na(broad_celltype_group), "unknown", broad_celltype_group) |> 
...
# set order of gene symbol based on celltype groups 
...
color_bar <- ggplot(dotplot_df, aes(x = gene_symbol, y = 1, fill = validation_group_annotation)) + 
  geom_tile() + 
  scale_fill_manual(values = plot_colors, breaks = levels(dotplot_df$validation_group_annotation)) +
  ggmap::theme_nothing() +
...
unique_heatmap_mtx <- unique_gene_df |> 
  dplyr::ungroup() |> 
  dplyr::select(gene_symbol, broad_celltype_group, mean_exp) |> 
...
unique_heatmap_mtx <- unique_heatmap_mtx[c(celltype_order, "unknown"), filtered_markers_df$gene_symbol]

# create annotation for heatmap
annotation <- ComplexHeatmap::columnAnnotation(
  group = filtered_markers_df$validation_group_annotation,
  col = list(
...
marker_gene_heatmap(unique_heatmap_mtx, annotation)
```

## Mean expression across all marker genes 

We will make the same plots but instead of looking at the expression of individual marker genes for all cell type groups, we will plot the mean expression of all marker genes for a specific cell type group. 
...
```{r}
# get annotation 
grouped_annotation <- ComplexHeatmap::columnAnnotation(
  group = unique(filtered_markers_df$validation_group_annotation),
  col = list(
    group = celltype_colors
  ),
  annotation_legend_param = list(
...
# first make a mtx to use for the heatmap with rows as cell types and marker gene groups as columns 
grouped_heatmap_mtx <- group_stats_df |> 
  dplyr::select(validation_group_annotation, broad_celltype_group, all_markers_mean_exp) |> 
  unique() |>
  dplyr::filter(validation_group_annotation %in% celltype_order) |> 
  tidyr::pivot_wider(
    names_from = validation_group_annotation,
    values_from = all_markers_mean_exp
  ) |> 
  tibble::column_to_rownames("broad_celltype_group") |>
  as.matrix()
```
...
```{r}
# make sure cell types are present in the right order
grouped_heatmap_mtx <- grouped_heatmap_mtx[c(celltype_order, "unknown"), celltype_order]
```



```{r, fig.height=7, fig.width=10}
marker_gene_heatmap(grouped_heatmap_mtx, grouped_annotation)
```
