...
consensus_results_files <- list.files(results_dir, pattern = "_consensus-cell-types\\.tsv\\.gz$", recursive = TRUE, full.names = TRUE) 
...
markers_df <- duckplyr::read_csv_duckdb(validation_markers_file) |> 
  duckplyr::as_duckdb_tibble()

validation_groups_df <- duckplyr::read_csv_duckdb(validation_groups_file) |> 
...
consensus_df <- duckplyr::read_csv_duckdb(consensus_results_files, options = list(sep = "\t", union_by_name = TRUE)) 
...
is_cell_line <- params$project_id %in% c("SCPCP000020", "SCPCP000024")
...
consensus_df <- consensus_df |> 
  ...
  # we only want genes that are in both and throw out extra consensus genes 
...
group_stats_df <- consensus_df |> 
  ...
  # remove any marker genes that don't have an assigned validation group
  tidyr::drop_na()
...
celltype_colors <-  colorRampPalette(RColorBrewer::brewer.pal(12, "Paired"))(length(celltype_groups))
...
```{r, eval=!is_cell_line, fig.height=15, fig.width=25}
