...
1:22 |>
  purrr::map(
    \(i) 
      do_Feature_mean(cell_type_df_sub, group.by = "second.level_annotation", feature = glue::glue("has_loss_chr", i, "q"))
) |>
...
1:22 |>
  purrr::map(
    \(i) 
      do_Feature_mean(cell_type_df_sub, group.by = "second.level_annotation", feature = glue::glue("has_dupli_chr", i, "q"))
) |>
# send into patchwork to make a single plot from the list of plots in 2 columns
patchwork::wrap_plots(ncol = 2)
```
...
c(1:12, 16:20) |>
  purrr::map(
    \(i) 
      do_Feature_mean(cell_type_df_sub, group.by = "second.level_annotation", feature = glue::glue("has_loss_chr", i, "p"))
) |>
# send into patchwork to make a single plot from the list of plots in 2 columns
patchwork::wrap_plots(ncol = 2)
```
...
c(1:12, 16:20) |>
  purrr::map(
    \(i) 
      do_Feature_mean(cell_type_df_sub, group.by = "second.level_annotation", feature = glue::glue("has_dupli_chr", i, "p"))
) |>
# send into patchwork to make a single plot from the list of plots in 2 columns
patchwork::wrap_plots(ncol = 2)
```
...
for(chr in unique(CNV_profile_loss$chromosome_arm)){
  loss_profile[[chr]] <- ggplot(CNV_profile_loss[CNV_profile_loss$chromosome_arm == chr,], 
