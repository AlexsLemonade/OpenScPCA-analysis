  ...
  purrr::map(
    ...
    \(sample_id) {
      input_file <- file.path(
        result_dir,
        sample_id,
        glue::glue("06_infercnv_HMM-i3_{sample_id}_reference-both.rds")
      )
...
- Combining label transfer and CNV inference we annotated 38/40 Wilms tumor samples in SCPCP000006. For the two remaining samples, we need to investigate inferCNV further:
    - SCPCS000190 only has 1 normal cell, which is insuffient as a reference for inferCNV
    - SCPCS000203 does not run properly through inferCNV; cells are removed from the inferCNV inference, and why requires further investigation
