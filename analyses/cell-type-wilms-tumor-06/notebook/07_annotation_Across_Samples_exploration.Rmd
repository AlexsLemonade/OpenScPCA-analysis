...
_Where `cnv.thr` and `map.thr` need to be discussed_
...
We extracted from the pre-processed and labeled `Seurat` object (that is the output of `06_infercnv_HMM-i3_SCPCS000169_reference-both.rds` saved in the `results` directory)
...
cell_type_df <- metadata$scpca_sample_id |>
  purrr::map(
    ...
    \(sample_id) {
     ...
     if(file.exists(file.path(result_dir, sample_id, paste0("06_infercnv_HMM-i3_", sample_id, "_reference-both.rds")))){
       
       srat <- readRDS(
        file.path(result_dir, sample_id, paste0("06_infercnv_HMM-i3_", sample_id, "_reference-both.rds"))
      )
...
#### CNV score
  cell_type_df$has_cnv_score <- rowSums(cell_type_df[,grepl("has_cnv_chr", colnames(cell_type_df))])
  cell_type_df$has_cnv_score[cell_type_df$has_cnv_score > params$cnv_threshold] <- "CNV"

  # we know that some immune cells might have false positive CNV 
  cell_type_df$has_cnv_score[cell_type_df$has_cnv_score %in% c(0:params$cnv_threshold)] <- "no CNV"
...
cell_type_df$first.level_annotation[  cell_type_df$compartment %in% c("fetal_nephron", "stroma") &
                                      cell_type_df$has_cnv_score == "no CNV"]                          <- "normal"

cell_type_df$first.level_annotation[  cell_type_df$compartment %in% c("endothelium", "immune") &
                                      (cell_type_df$compartment.score > params$predicted.celltype.threshold |
                                      cell_type_df$has_cnv_score == "no CNV") ]                        <- "normal"

# Define cancer cells
cell_type_df$first.level_annotation[  cell_type_df$compartment %in% c("fetal_nephron", "stroma") &
                                      cell_type_df$has_cnv_score == "CNV" ]                            <- "cancer"
...
cell_type_df$second.level_annotation[  cell_type_df$compartment %in% c("fetal_nephron") &
...
cell_type_df$second.level_annotation[  cell_type_df$compartment %in% c("endothelium") &
                                      (cell_type_df$compartment.score > params$predicted.celltype.threshold |
                                      cell_type_df$has_cnv_score == "no CNV") ]                        <- "endothelium"

cell_type_df$second.level_annotation[  cell_type_df$compartment %in% c("immune") &
                                      (cell_type_df$compartment.score > params$predicted.celltype.threshold |
...
- Combining label transfer and CNV inference we annotated 37/40 Wilms tumor samples in SCPCP000006.

- For the three remaining samples, we need to check why we do not have the output of `06_infercnv.R --HMM i3 --reference both`. 
It might be that we don't have enough normal cell for the reference. 
