...
- `table` is the name of the table containing metadata and feature expression (counts) per cells
- `group.by` is the name of the metadata to group the cells
- `feature` is the name of the gene to average the expression
...
do_Feature_boxplot <- function(df, group.by, feature, split.by ){
  ...
  df <- df %>%
    mutate(!!sym(group.by), group = factor(!!sym(group.by), levels = unique(!!sym(group.by))))
...
p <- ggplot(df, 
    aes(x = !!sym(group.by), y = !!sym(feature), fill = !!sym(group.by))
  ) + 
  geom_boxplot(size = 0.5, size.outlier = 0.25) +
  facet_wrap(vars(!!sym(split.by)), scales = "free", ncol = 4) + 
  theme_bw() +
  theme(
...
kidney_count_df <- cell_type_df |>
  ...
  dplyr::mutate(
    ...
    kidney = round(kidney/total, digits = 4)*100, 
    not_kidney = round(not_kidney/total, digits = 4)*100) |>
