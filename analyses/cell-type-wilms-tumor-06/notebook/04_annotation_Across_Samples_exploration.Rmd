...
cell_type_df <- merge(cell_type_df, metadata, by.x = "sample_id", by.y = "scpca_sample_id")

cell_type_df$pass_mapping_QC <- FALSE 
cell_type_df$pass_mapping_QC[cell_type_df$compartment_score > params$mapping_score_thr] <- TRUE
```
...
```{r fig.width=10, fig.height=4, out.width='100%'}
do_Feature_densityplot <- function(df, group.by, feature, split.by = NULL  ){
...
Some cells have an excellent mapping score (>0,85) while other performed worth. Let's have a look at the repartiction per patient. 
