...
##########################################################################################################
In this notebook, we will test the human fetal kidney atlas from Stewart et al. using the Azimuth workflow. 
##########################################################################################################
...
This notebook has two main outputs

#### Azimuth compatible reference 

The reference data are downloaded from the `params$ref_url` and transiently saved in the `scratch`folder.
The azimuth compatible fetal kidney reference will be saved in the `results` folder from the module, `reference` subfolder.

Please note that the creation of the Azimuth reference cannot be done within RMarkdown. 
We thus set `include = FALSE` to the corresponding chunk.
Please, before running this code either : 

- download the azimuth compatible reference from S3 bucket or
- run manualy chunk by chunk the report to create and save the reference once.
...
#### The html notebook saved afetr rendering this notebook (knit).
Will be saved in the `notebook/00-reference` directory. 

## Using the Human Kidney Atlas - Fetal as a reference

### Create a azimuth compatible reference

Note: not shown here because the `AzimuthReference` function do not allow knitr.
The reference should be build "manually" prior to knitr the report.  

```{r create_ref, eval=FALSE, fig.height=7, fig.width=12, message=FALSE, warning=FALSE, include=FALSE, out.width='100%'}
...
data <-  RunAzimuth(srat, path_to_output, assay = 'RNA')

```

```{r fig.height=8, fig.width=8, warnings=FALSE}

d1 <- DimPlot(data, reduction = "umap", dims = c(1,2), group.by = "predicted.compartment", label = TRUE, repel = TRUE) + NoLegend()
d2 <- DimPlot(data, reduction = "umap", dims = c(1,2), group.by = "predicted.cell_type", label = TRUE, repel = TRUE) + NoLegend()
