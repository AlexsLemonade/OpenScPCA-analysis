...
In addition, the upper limit for the color scale has been censored to a maximum of 6.

```{r}
# Temporary to develop code for next section
annotation_df <- readr::read_tsv(params$annotations_tsv) |>
  dplyr::filter(cell_type_assignment != "Unknown") |>
  # recode cell types to match what NBAtlas uses for their marker genes
  dplyr::mutate(
    dotplot_celltype = dplyr::case_when(
      stringr::str_detect(cell_type_assignment, "monocyte") ~ "Monocyte", 
      stringr::str_detect(cell_type_assignment, "cDC") ~ "cDC", 
      stringr::str_detect(cell_type_assignment, "Neuroendocrine") ~ "Neuroendocrine", 
      .default = cell_type_assignment
    )
  )
...
total_cells_df <- annotation_df |>
  dplyr::count(dotplot_celltype, name = "total_cells") |>
  # remove infrequent cell types
  dplyr::filter(total_cells > 10) |>
  dplyr::mutate(y_label = glue::glue("{dotplot_celltype} ({total_cells})")) |>
  # order by count, but put Unknown at the end
  dplyr::arrange(desc(total_cells)) |> 
  # amazing new dplyr trick; only this row is TRUE, so it gets moved to the last row
  dplyr::arrange(dotplot_celltype == "Unknown") |>
  # the dotplot function expects this column name
  dplyr::rename(label_recoded = dotplot_celltype)
...
generate_dotplot(
  ...
  celltype_df = annotation_df |> dplyr::rename(label_recoded = dotplot_celltype),
