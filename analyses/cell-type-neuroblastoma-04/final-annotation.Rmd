...
First we prepare the data frame for annotation.
...
Only cell types with at least 10 detected cells are shown, and cells without an assigned label are not shown.
...
input_dotplot_df <- final_annotation_df |>
  ...
  # keep only cells that appear > 10 times
  dplyr::add_count(final_label) |>
  dplyr::filter(n > 10) |>
...
# placing this in eval in chunk settings makes the chunk
# ignore width and height, so we use an if
if (!(params$testing)) {
  generate_dotplot(
