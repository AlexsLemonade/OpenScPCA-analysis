...
```{r}
# get top 10??? upregulated marker genes per cell type
nbatlas_markers_df <- readr::read_tsv("../references/nbatlas-marker-genes.tsv") |>
  ...
  dplyr::ungroup() |>
  dplyr::filter(rank_lfc <= 10) |>
  harmonize_celltypes(NBAtlas_label, NBAtlas_label_recoded) |>
  dplyr::select(NBAtlas_label_recoded, gene_symbol, ensembl_gene_id, avg_log2FC)
```
...
all_info_df <- celltype_recoded_df |>
  ...
  dplyr::left_join(nbatlas_markers_df, by = "ensembl_gene_id", relationship = "many-to-many")
```
...
# bar order - NBAtlas
nbatlas_order <- total_cells_df$label_recoded

# x axis marker genes - nbatlas
nbatlas_markers_df <- nbatlas_markers_df |>
  dplyr::filter(ensembl_gene_id %in% expressed_markers) |>
  dplyr::mutate(
    NBAtlas_label_recoded = factor(NBAtlas_label_recoded, levels = nbatlas_order)
  ) |>
  dplyr::arrange(NBAtlas_label_recoded)
...
group_stats_df <- all_info_df |>
  ...
  dplyr::group_by(label_recoded, NBAtlas_label_recoded, ensembl_gene_id) |>
  dplyr::summarize(
  ...
  # remove lowly expressed marker genes and any that aren't present in final annotations
  dplyr::filter(
    mean_exp > 0,
    percent_exp > 10,
  ...
  dplyr::left_join(
    nbatlas_markers_df,
    by = c("ensembl_gene_id", "NBAtlas_label_recoded"),
...
dotplot <- ggplot(dotplot_df, aes(y = y_label, x = gene_symbol, color = mean_exp, size = percent_exp)) +
  ...
  facet_grid(cols = vars(NBAtlas_label_recoded), scales = "free", space = "free") +
  theme_classic() +
  theme(
  ...
  labs(
    x = "NBAtlas cell type marker genes",
    y = "SingleR label",
...
color_bar <- ggplot(dotplot_df, aes(x = gene_symbol, y = 1, fill = NBAtlas_label_recoded)) +
  geom_tile() +
  facet_grid(cols = vars(NBAtlas_label_recoded), scales = "free", space = "free") +
  scale_fill_manual(values = celltype_pal, breaks = levels(dotplot_df$NBAtlas_label_recoded)) +
  ggmap::theme_nothing() +
  theme(
    strip.background = element_rect(fill = "transparent", color = NA),
