...
To facilitate this, the `SingleR` labels from `NBAtlas` were grouped together to both a) mirror (where possible) the broad consensus labels, and b) to make some visualizations more manageable.
...
Also, note that the `myeloid`-labeled SingleR cells are _only_ neutrophils, but the consensus myeloid cagetory contains additonal cell types.
...
We'll show the top-5 highest log2FC marker genes per cell type for validation.
