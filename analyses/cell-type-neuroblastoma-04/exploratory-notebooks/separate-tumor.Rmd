...
When training a model, SingleR performs pairwise comparisons to identify marker genes as the top 10 upregulated genes for each label compared to each other label.
...
Next, we'll look at the NE marker genes between references.

For a quick summary, we'll ask: How many of the NE markers in the combined model (out of 10 total per cell type comparison!) are _also included_ in the separated model's NE (or NE-tumor) marker gene lists?
...
This minimal exploration does not suggest that there are salient differences between the two NE categories in the separated model.
...
In the next section, we'll look at the numbers a bit more closely.
...
In the separated reference, for cells that are labeled either `NE` or `NE-tumor` (considering the `pruned.labels` specifically), which label has the _second highest score_?
We expect that the second highest score will usually be the other `NE` label if these cell types are indeed highly similar.


Note that there were this many total NE and NE-tumor cells identified by the separated model:
```{r}
separated_ne_df <- singler_results_separated |>
  as.data.frame() |>
  tibble::rownames_to_column("barcodes") |>
  # keep only cells labeled with one of the NE categories
  dplyr::filter(stringr::str_detect(pruned.labels, "Neuroendocrine"))
nrow(separated_ne_df)
```

The table below shows counts for how many times the given label is the second-highest score:
...
```{r}
# tidy the scores into long format
scores_df <- singler_results_separated$scores |>
  as.data.frame() |>
  dplyr::mutate(barcodes = rownames(singler_results_separated)) |>
  tidyr::pivot_longer(
    -barcodes,
    names_to = "score_label",
    values_to = "score"
  )


# This dataframe grabs the labels associated with delta.next, aka the label for the 2nd highest score
next_label_df <- scores_df |>
  dplyr::group_by(barcodes) |>
  # use -score to go in descending order and then keep only the SECOND rank
  dplyr::mutate(score_rank = rank(-score)) |>
  dplyr::filter(score_rank == 2) |>
  dplyr::ungroup() |>
  dplyr::select(barcodes, delta.next_label = score_label)


# This dataframe grabs the actual scores associated with the assigned pruned.labels
full_scores_df <- scores_df |>
  # bring in the real labels and delta.next scores
  dplyr::inner_join(
    data.frame(
      barcodes = rownames(singler_results_separated),
      singler_label = singler_results_separated$pruned.labels,
      delta.next = singler_results_separated$delta.next
    ),
    by = "barcodes"
  ) |>
  # the score whose label matches the assigned label is the score we want
  dplyr::filter(score_label == singler_label) |>
  dplyr::select(barcodes, singler_label_score = score, singler_label, delta.next) |>
  # bring in the delta.next_label
  dplyr::inner_join(
    next_label_df,
    by = "barcodes"
  ) |>
  unique() |>
  # Finally, add a column for the delta.next / singler_label_score
  dplyr::mutate(delta.next_rel_difference = (delta.next / singler_label_score))
full_scores_df |>
  dplyr::filter(delta.next_fraction > 1)

ggplot(full_scores_df) +
  aes(x = singler_label, y = delta.next_fraction) +
  geom_boxplot() +
...
next_label_barcodes <- next_label_df$barcodes


singler_results_separated |>
  as.data.frame() |>
  tibble::rownames_to_column("barcodes") |>
  dplyr::select(barcodes, pruned.labels, delta.next) |>
  ggplot() +
  aes(x = pruned.labels, y = delta.next) +
  geom_boxplot() +
  theme(axis.text.x = element_text(angle = 90))

dplyr::add_count(delta.next_label)
# format for printing
dplyr::mutate(second_highest_delta_label = stringr::str_remove(cell_type, "scores\\.")) |>
  dplyr::arrange(desc(n)) |>
  dplyr::select(
    assigned_label = pruned.labels,
    second_highest_delta_label,
  )
```
