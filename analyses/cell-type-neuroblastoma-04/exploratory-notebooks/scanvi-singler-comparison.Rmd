...
We'll look at the minimum, maximum, and median posterior probabilities across libraries in a table.
...
scanvi_df |>
  ...
  dplyr::summarize(
    ...
    min_posterior = min(scanvi_posterior), 
    median_posterior = median(scanvi_posterior), 
    max_posterior = max(scanvi_posterior)
  ) |>
  dplyr::arrange(library_size)
```

Across libraries, the median posterior probability is always exceptionally high at >=0.99, which suggests `scANVI` confidence is roughly the same across libraries.
...
This will throw out a bit under 2% of cells:
