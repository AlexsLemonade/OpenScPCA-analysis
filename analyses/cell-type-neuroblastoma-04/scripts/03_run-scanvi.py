...
def main() -> None:
    ...
    all_present_reference = all(
        col in reference.obs.columns for col in reference_expected_columns
    )
    ...
    all_present_query = all(
        col in query.obs.columns for col in expected_covariate_columns
    )
    ...
    exclude_libraries = (
        [id.strip() for id in arg.exclude_libraries.split(",")]
        if arg.exclude_libraries
        else []
    )
    if len(exclude_libraries) > 0:
        all_present = all(
            [id in query.obs["library_id"].cat.categories for id in exclude_libraries]
        )
        if not all_present:
            print(
    ...
    adata_integrated.obs[SCANVI_PREDICTIONS_KEY] = scanvi_query.predict(
        adata_integrated
    )
