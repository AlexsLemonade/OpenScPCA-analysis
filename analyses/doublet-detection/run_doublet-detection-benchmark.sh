...
if [[ $TEST -eq 1 ]]; then
    ...
    curl -OL https://github.com/AlexsLemonade/OpenScPCA-analysis/raw/c9aad22478c3888089bcff9d6a04f011844c5256/analyses/doublet-detection/benchmark-test-data/data.zip
    unzip data.zip -d ${DATA_DIR}/raw
    rm data.zip
else
    # Download and unzip `real_datasets.zip` archive from https://doi.org/10.5281/zenodo.4562782
    curl -OL https://zenodo.org/records/4562782/files/real_datasets.zip
