...
# If scrublet fails, then a TSV with NA values is exported.
...
def run_scrublet(adata: anndata.AnnData, random_seed: int) -> pandas.DataFrame:
    ...
    results = {
        ...
        "scrublet_prediction": predicted_doublets_str,
    }
...
def main() -> None:
    ...
    if not args.output_file.name.endswith(".tsv"):
        print("The output TSV file must end in `.tsv`.", file=sys.stderr)
        arg_error = True
