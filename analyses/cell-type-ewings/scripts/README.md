...
The input `SingleCellExperiment` object is filtered to remove any cells that are labeled as tumor and present in the reference output from `01-generate-tumor-ref.R`. 
The remaining cells are then run through `SingleR` using both the reference from `01-generate-tumor-ref.R` and `BlueprintEncodeData` from `celldex` as the references. 
Note that any tumor cells from the library being annotated are removed from the tumor reference object prior to running `SingleR`. 
