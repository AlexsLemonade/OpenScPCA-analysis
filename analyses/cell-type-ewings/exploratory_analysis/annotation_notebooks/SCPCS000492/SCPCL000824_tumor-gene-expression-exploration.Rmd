...
The gene set scores correspond to the mean expression of all genes in a given gene set for a single cell with no scaling. 
The normalized gene expression was used as input to calculating the mean. 
...
One caveat is that `AUCell` relies on having a bimodal distribution in gene set expression, so I expect we may need to do a similar thing where we use cutoffs determined in `SCPCL000822` to call cells in `SCPCL000824`.  
...
Looking at the AUC across both samples for all gene sets, we see that they are in the same range of values and tend to mirror each other. 
The main difference here is the lack of bimodal distribution in `SCPCL000824`. 

Let's use the threshold identified by `AUCell` for `SCPCL000822` to identify tumor cells in `SCPCL000824`.  
