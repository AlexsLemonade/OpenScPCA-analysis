...
4. UMAP embeddings for each library is pulled out into a data frame and combined with `SingleR` annotations, cluster assignments, and marker gene expression for all marker gene sets.
...
Now we will look at the marker gene expression across all clusters for each sample. 
First using a density plot and then a heatmap. 
