...
I would expect to see some sort of bimodal distribution separating cells that do and do not have expression of the marker gene. 
...
We would expect that cells we are labeling as tumor cells have expression of the tumor marker genes. 
...
marker_plot_df <- celltype_df |>
  dplyr::mutate(gene_expressed = dplyr::if_else(transformed_gene_expression > 0, TRUE, FALSE)) |>
  dplyr::select(gene_symbol, gene_expressed, sum_classification)

ggplot(marker_plot_df, aes(x = sum_classification, fill = gene_expressed)) +
  geom_bar(stat = "count") +
...
Looking at this might help give us an idea of genes that are more specifically expressed in tumor cells. 
Genes like `ADGRG2` and `HES1` do not seem specific to tumor cells. 
There are normal and tumor cells that express the gene and not very many tumor cells actually have expression of `ADGRG2.` 
`STEAP1` and `NROB1` although not expressed in very many tumor cells, look to be specific to tumor cells which is promising. 

`CD99`, `CAV1`, and `CCND1` are expressed in a high proportion of tumor cells but are also expressed in quite a few normal cells. 
...
`KDSR`, `MAPT`, `NKX2-2`, `PAPPA`, and `PRKCB` all seem to have a high proportion of tumor cells expressing the gene and a low proportion of normal cells that express the gene. 
`PRKCB` and `MAPT` are also 2 of the top genes identified using the marker analysis. 

Almost all genes have at least some expression in normal cells. 
Perhaps those cells should also be categorized as tumor cells, or they just have some baseline expression in normal cells which is possible. 
...
We also see that normal cells contain some smooth muscle cells, but mostly are fibroblasts and endothelial cells. 
...
Let's look at expression of marker genes in these endothelial cells compared to cells that are categorized as smooth muscle cells. 
