...
# get clusters for a specified value of k
...
# calculated the ari between original clusters and shuffled clusters 20 times
...
cluster_celltype_heatmap <- function(cluster_classification_df,
  ...
  make_heatmap_list(jaccard_df_list, column_title = glue::glue("{id}-clusters"), legend_match = "k_5")
}
...
# look at clustering results across all params
...
```{r}
# summarize the silhouette width and purity for each nn_param used
summary_stats_df <- all_stats_df |>
  dplyr::group_by(library_id, nn_param) |>
  dplyr::summarise(
...
ggplot(summary_stats_df, aes(x = nn_param, y = mean_width)) +
  geom_point() +
...
ggplot(summary_stats_df, aes(x = nn_param, y = mean_purity)) +
  geom_point() +
  facet_wrap(vars(library_id),
    scales = "free_y"
  ) +
  labs(y = "Average cluster purity")
```
...
However, for the other values of K, there doesn't seem to be any striking difference that are obvious by eye.  
...
marker_gene_columns |>
  purrr::map(\(column){
    genelist_name <- stringr::str_remove(column, "_sum")

    ggplot(cluster_umap_df, aes(x = UMAP1, y = UMAP2, color = !!sym(column))) +
      geom_point(alpha = 0.5, size = 0.5) +
      facet_grid(cols = vars(library_id)) +
      theme(aspect.ratio = 1) +
