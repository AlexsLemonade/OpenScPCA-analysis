...
jaccard_mtx <- make_jaccard_matrix(
  all_info_df,
...
Any cells that are to the left of these cutoffs will be classified as tumor. 
...
- `tumor EWS-high`: High expression of `aynaud-ews-targets` (>0.025 AUC) and `staege` (>0.025 AUC)
...
all_info_df <- all_info_df |> 
  dplyr::mutate(
    final_annotation = dplyr::case_when(
      ...
      `auc_aynaud-ews-targets` > 0.025 & auc_staege > 0.025 ~ "tumor EWS-high",
...
For plotting purposes, we will only show the top 10 cell types with fibroblast being the last cell type. 
