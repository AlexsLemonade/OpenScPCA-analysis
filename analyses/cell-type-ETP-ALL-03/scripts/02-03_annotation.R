...
running_scType <- function(xls, tissue, annot.obj, assay = "RNA", thres = 4){
  gs_list <- gene_sets_prepare(xls, tissue) #prepare gene sets
  ...
  for(j in unique(sctype_scores$cluster)){
    ...
    annot.obj@meta.data$sctype_classification[annot.obj@meta.data$leiden_clusters == j] = as.character(cl_type$type[1])
  }
  return (list(annot.obj, cL_results))
}

plot_modulescore <- function(xls, tissue, seu, sample.name){
  gs_list <- gene_sets_prepare(xls, tissue) #prepare gene sets
  ...
  ggsave(file.path(out_loc,"plots",paste0(sample.name,"_features_dotplot.pdf")), width = 7, height = 7)
}

run_annot <- function(ind.sam){
  seu <- readRDS(file.path(out_loc,"results/rds",paste0(ind.sam,".rds")))
  ...
  res <- running_scType(xls, tissue, seu, thres = 10)   #cell type annotation
  seu$lowConfidence_annot <- res[[1]]$sctype_classification #seurat object with 'sctype_classification' metadata added
  res <- running_scType(xls, tissue, seu)
  seu <- res[[1]]
  #res[[2]] - A table with top 10 cell types with the highest scores for each cluster
  write.table(res[[2]], file = file.path(out_loc,"results",paste0(ind.sam,"_sctype_top10_celltypes_perCluster.txt")),
              row.names = F, sep = "\t", quote = F)
  p1 <- DimPlot(seu, reduction = "Xumap_", group.by = "sctype_classification", cols = ct_color) +
    ggtitle(paste0(ind.sam,": cell type"))
  p2 <- DimPlot(seu, reduction = "Xumap_", group.by = "lowConfidence_annot", cols = ct_color) +
    ggtitle(paste0(ind.sam,": low confidence annotation"))
  p1 + p2
  ggsave(file.path(out_loc,"plots",paste0(ind.sam,"_celltype.pdf")), width = 10, height = 5)

  plot_modulescore(xls, tissue, seu, ind.sam)
...
sampleID <- unique(metadata$scpca_sample_id)
# DB file
db <- file.path(out_loc,"Azimuth_BM_level1.csv")
xls <- openxlsx::buildWorkbook(read.csv(db, header = T))
