...
if (opts$testing) {
  ks.cut_param <- 0.001 # smaller value for test data
} else {
...
plot_modulescore <- function(gs_list, seu, sample.name) {
  for (i in 1:length(gs_list$gs_positive)) {
    seu <- AddModuleScore(
      object = seu, name = paste0(gsub(" ", "", names(gs_list$gs_positive[i])), "_Features"),
...
run_annot <- function(ind.lib, ks_cut = 0.1) {
  ...
  write.table(res[[2]],
    file = file.path(out_loc, "results", paste0(ind.lib, "_sctype_top10_celltypes_perCluster.txt")),
