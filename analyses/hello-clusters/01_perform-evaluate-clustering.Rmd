...
This notebook will demonstrate use of `rOpenScPCA` with the `SCPCS000001` sample from project `SCPCP000001`, which is assumed present in the `OpenScPCA-analysis/data/current/SCPCP000001` directory.
...
Default use of this function is shown below.
...
All examples will use the SCE object for demonstration, but a Seurat object can be provided instead and will provide the same results.
Again, as needed, you may need to specify the `pc_name` argument if the principal components do not use the object type's default name (`"PCA"` for SCE and `"pca"` for Seurat).
...
- Optionally, `pc_name` if your PCA matrix does not have the default name for the object type (`"PCA"` for SCE and `"pca"` for Seurat)
...
- `other`: The closet cluster to the cell besides the cluster to which it belongs, as used in the silhouette width calculation
...
ggplot(silhouette_results) +
  ...
  geom_vline(xintercept = mean_silhouette_width, color = "red", size = 1)
```

Looking at the full distribution of silhouette width values, we see most are positive which is encouraging, but the values are mostly below 0.5, and the mean is around 0.2 which is somewhat low.
...
- `maximum`: The cluster with the highest proportion of observations neighboring the cell
...
ggplot(purity_results) +
  aes(x = purity) +
  geom_density(fill = "lightblue") +
  geom_vline(xintercept = mean_purity, color = "red", size = 1)
```
...
With this view, we see that clusters 1, 3, and 6 all have average values below 0.75, but the rest are higher, in particular clusters 11, 12, and 13.
...
Since this function uses random sampling when performing bootstrapping, it's important to pass in a random seed for reproducibility.
...
All bootstrap replicates produced values around 0.7, which is relatively close to 1. 
This result suggests that these clusters are fairly stable.
...
Objects from the ScPCA Portal already contain a column called `cluster` with results from an automated clustering which was _not evaluated_ or parameterized according to the dataset. 
To avoid ambiguity with these new clustering results, you may want to rename columns.

The subsequent examples will show you how to save the `cluster_results` data frame to a given object, as an example.
We'll begin by adding the prefix `ropenscpca_` to all column names in the `cluster_results` data frame before adding them to the given object.
...
Follow these steps save the clustering results to an SCE object's `colData`:
...
seurat_obj <- AddMetaData(seurat_obj, cluster_results_openscpca)
