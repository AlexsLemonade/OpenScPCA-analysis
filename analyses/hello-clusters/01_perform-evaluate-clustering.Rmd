...
* Perform clustering
* Calculate QC metrics on clustering including:
  * Silhouette width
  * Neighborhood purity
  * Cluster stability, as measured with the Adjusted Rand Index
* Calculate QC metrics obtained with other tools, such as `Seurat`
...
# Moving forward, we'll use this version for most examples
pca_matrix <- reducedDim(sce, "PCA")


# Extract the PCA matrix from a Seurat object
...
We previously created a Seurat object called `seurat_obj` with clusters:


```{r show seurat clusters}
...
These clusters were calculated using `bluster`, the same tool that `rOpenScPCA` uses, with the following parameters:
...
When running `calculate_stability()`, we'll specify the same parameters originally used to build the clusters.
We only recommend evaluating clusters with `calculate_stability()` if you know the original parameters used.
