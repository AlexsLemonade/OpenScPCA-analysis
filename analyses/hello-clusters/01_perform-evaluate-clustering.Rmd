...
In this section, we'll present examples of how you can calculate the silhouette width, neighborhood purity, and cluster stability from existing cluster assignments.
...
To demonstrate this, we'll first create a Seurat object from our SCE object. 
We'll use the function `rOpenScPCA::sce_to_seurat()` for this, setting `use_symbols = FALSE` to preserve Ensembl ids as the feature names rather than converting to gene symbols.
...
The `rOpenScPCA::sce_to_seurat()` function will carry over the existing SCE column `cluster` into the Seurat object, which we'll use here for demonstration. 
If you calculate clusters within the Seurat ecosystem, you may need to use a different column name here, e.g. `seurat_clusters`.

```{r seurat cluster}
head(seurat_obj$cluster)
```

To calculate QC metrics on these clusters, we'll need two pieces of information:

- The PCA matrix
- A data frame with columns `cell_id` and `cluster`

We'll create those here:

```{r prepare seurat input}
# Extract the PCA matrix from the Seurat object
...
We can run evaluation functions using this data frame and the PCA matrix from the object.
...
head(scpca_purity_df)
```

When running `rOpenScPCA::calculate_stability()`, we'll specify the same parameters originally used to build the clusters.
We only recommend evaluating clusters with `rOpenScPCA::calculate_stability()` if you know the original parameters used.
...
We show examples of this below with `rOpenScPCA::calculate_clusters()`, but this will also work for any of the evaluation functions as well:
...
cluster_results_df <- rOpenScPCA::calculate_clusters(seurat_obj)
cluster_results_df
```
