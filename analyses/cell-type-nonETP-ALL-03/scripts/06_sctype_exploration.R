...
plot_Bscore <- function(ind.lib){
  ...
  sctype.score <- read.table(file.path(out_loc,"results",paste0(ind.lib,"_sctype_scores.txt")), 
                             sep = "\t", header = T)
  #shown from the B cells annotated by SingleR and dotplot (B_Features1)
  special.lib <- c("SCPCL000077","SCPCL000704","SCPCL000710")
  special.clus <- c(12,5,7)
  if (ind.lib %in% special.lib){
    Bcell.names <- colnames(seu)[which(seu$leiden_clusters==special.clus[match(ind.lib,special.lib)])]
  }else if (ind.lib == "SCPCL000082"){
    return()
  }else{
    Bcell.names <- colnames(seu)[which(seu$sctype_classification=="B")]
  }
