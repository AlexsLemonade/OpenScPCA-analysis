...
library_diagnosis_df <- all_library_diagnosis_df |>
  # don't consider cell line or multiplexed in this notebook
  # the multiplexed samples were not processed with SCimilarity in OpenScPCA-nf
  dplyr::filter(!is_cell_line, !is_multiplexed)
```
...
consensus_df <- consensus_files |>
  ...
  purrr::imap(
    \(file, id) {
      ...
      consensus_df <- readr::read_tsv(file, guess_max = 15000)
      
      # ensure we have the most up-to-date files - aka, with SCimilarity
      stopifnot("Missing SCimilarity in the consensus files" = "scimilarity_celltype_annotation" %in% colnames(consensus_df))
...
```{r}
usable_counts_df <- library_counts_df |>
...
```{r}
usable_libraries_df <- library_counts_df |>
  dplyr::filter(reference >= params$n_reference_threshold) 

fewer_query_df <- usable_libraries_df |>
  dplyr::filter(query < params$n_reference_threshold)

fewer_query_df
```

There are five libraries with an order of magnitude more reference than query cells; let's look more closely at the validation groups present to get a sense.

```{r}
consensus_df |>
  dplyr::filter(scpca_library_id %in% fewer_query_df$scpca_library_id) |>
  dplyr::left_join(validation_group_df, by = c("consensus_ontology", "consensus_annotation")) |>
  dplyr::count(scpca_library_id, validation_group_annotation) |>
  dplyr::arrange(scpca_library_id)
```

These libraries are dominated by immune cell types with very few Unknown/tumor cells, but it's still worth running them through `inferCNV` while being aware that we _may_ experience issues with these samples.
...
plot_df <- query_reference_df |>
  dplyr::filter(scpca_library_id %in% usable_libraries_df$scpca_library_id) |>
  dplyr::left_join(validation_group_df, by = c("consensus_ontology", "consensus_annotation")) |>
  dplyr::select(
    scpca_project_id, 
    scpca_library_id, 
...
We are able to run about 50\% of libraries through `inferCNV`, based on consensus cell types derived from `SingleR`, `CellAssign`, and `SCimilarity` labels.
Most broad diagnosis groups are represented, except `Mixed phenotype acute leukemia` (n=18) and a handful of other solid tumors (n=3 among Gastrointestinal stromal tumor and Hepatoblastoma).
All eligible projects are represented as well.

Note that it was not possible to fully assess multiplexed libraries here since they did not undergo `SCimilarity` (and hence their consensus cell types are not updated) inference in `OpenScPCA-nf`. 
They will be included when running through `scpca-nf`, however, assuming they have sufficient reference cells; it is important to bear in mind that these libraries would effectively be using pooled references in `inferCNV`, since we did not demultiplex libraries.
