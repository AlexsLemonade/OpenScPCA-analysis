...
```{r}
all_sample_diagnosis_df <- metadata_files |>
  purrr::map(
    \(file) {
      readr::read_tsv(file) |>
        dplyr::select(contains("scpca"), diagnosis, is_cell_line, is_multiplexed)
  }) |>
  dplyr::bind_rows() |>
...
total_samples <- nrow(all_sample_diagnosis_df) # needed for later math

sample_diagnosis_df <- all_sample_diagnosis_df |>
...
```{r}
sum(sample_counts_df$reference >= params$n_reference_threshold) / total_samples
```
