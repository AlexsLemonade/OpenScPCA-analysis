...
validation_group_df <- readr::read_tsv(validation_url)
```
...
In total, we can run this fraction of ScPCA samples:
...
This sample indeed is dominated by immune cell types.
This sample may just have very low tumor purity, but I think it's still fine to run through `inferCNV`.
...
All broad diagnosis groups are represented.
