...
There are five library with an order of magnitude more  7x as many reference as query cells; let's look more closely at the validation groups present to get a sense.
...
We should also check the eligible `SCPCP000009` libraries to see if they are multiplexed:

```{r}
library_diagnosis_df |>
  dplyr::filter(scpca_project_id == "SCPCP000009", scpca_library_id %in% usable_libraries_df$scpca_library_id) |>
...
We are able to run about 51% of libraries through `inferCNV`, based on consensus cell types derived from `SingleR`, `CellAssign`, and `SCimilarity` labels.
...
Note that we are including multiplexed libraries from project `SCPCP000009` here as well; 12 libraries from that project from are eligible, and all but of of those are multiplexed.
