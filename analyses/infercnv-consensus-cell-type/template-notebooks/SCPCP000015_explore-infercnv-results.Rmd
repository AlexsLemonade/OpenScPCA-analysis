...
has_cnv_df <- infercnv_df |>
  ...
  dplyr::mutate(
    celltype_n = glue::glue("{celltype} (n={celltype_count})"),
    cell_group_n = glue::glue("{cell_group} (n={cell_group_count})")
  )

# Set the celltype_n factor order based on label, not frequency
# We don't know the exact labels due to `(n=)`, so use grep
has_cnv_df$celltype_n <- as.factor(has_cnv_df$celltype_n)
present_values <- levels(has_cnv_df$celltype_n)
has_cnv_df$celltype_n <- forcats::fct_relevel(
  has_cnv_df$celltype_n,
  c(
    grep("reference", present_values, value = TRUE),
    grep("tumor", present_values, value = TRUE),
    grep("other", present_values, value = TRUE)
  )
)
```
...
right_plot <- ggplot(has_cnv_df) +
  aes(y = forcats::fct_rev(celltype_n), x = total_cnv_per_cell, fill = celltype_n) +
  ggridges::geom_density_ridges2(alpha = 0.7, scale = 0.85) +
...
# drop cells from other libraries, and update data frame for plotting
query_has_cnv_df <- has_cnv_df |>
  dplyr::filter(library_id == params$library_id) |>
  dplyr::add_count(celltype) |>
  # remove really low count cell types
  dplyr::filter(n >= 3) |>
  # add column with cell type counts, and arrange in order
  dplyr::mutate(
    celltype_n = glue::glue("{celltype} (n={n})"),
    celltype_n = forcats::fct_infreq(celltype_n),
    celltype_n = forcats::fct_rev(celltype_n)
  )

ggplot(query_has_cnv_df) +
  aes(x = total_cnv_per_cell, y = celltype_n) +
  ggridges::geom_density_ridges2(alpha = 0.7, scale = 0.85)
```
