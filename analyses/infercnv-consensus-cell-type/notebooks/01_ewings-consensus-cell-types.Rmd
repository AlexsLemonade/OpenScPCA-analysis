...
We might want to investigate these cells further, but at least we should probably exclude them as "normal" cells for `inferCNV` since their true status is bit unclear.


## Conclusions

* There are sufficient immune cells to pool together to run `inferCNV` specifying them as a normal reference
* We could also consider using only `macrophage` cells, or combined `macrophage` and all `T cell` types together, if we wanted to assess any potential effect of limit the types of immune cells feeding into `inferCNV`. 
