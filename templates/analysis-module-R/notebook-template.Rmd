...
Replace this text with your own introduction, and be sure to update the `title` and `author` fields at the top of the document.
...
You may not know all of the paths you need when you start a notebook, but once you do, keeping them all in one place can be helpful, both for yourself in the future and for other users of the notebook.


### Base directories

In the chunk below, we included some common directories that you might need, finding their locations using the very useful `rprojroot` package.
...
# This module's base path, found by the presence of an `renv` directory
module_base <- rprojroot::find_root(rprojroot::is_renv_project)
# If you are not using `renv`, you might use something like the following instead:
# module_base <- file.path(repository_base, "analyses", "{module_name}")
```
